...
def main():
    ...
    for _ in range(120):
        ...
        if any(
            e["status"]["observed_state"]["state_type"] == "running" for e in executions
        ):
            break
        time.sleep(0.5)
    else:
        raise RuntimeError("No running execution for the new version")
    for e in executions:
        print(
            "Cloud execution:",
            e["id"],
    ...
    time.sleep(1)  # let the new listener bind before the old one's port is reused
